const REDIS_URL = process.env.UPSTASH_REDIS_REST_URL || 'https://pumped-heron-38885.upstash.io';
const REDIS_TOKEN = process.env.UPSTASH_REDIS_REST_TOKEN || 'AZf1AAIncDEzMjRhYmQyYmI1MmQ0YTE5ODAyY2Q0ZDdmOGQzMTk0ZXAxMzg4ODU';

async function redis(...args) {
  const r = await fetch(`${REDIS_URL}/${args.map(a => encodeURIComponent(String(a))).join('/')}`, {
...
async function setRoom(code, room) {
  room.ts = Date.now();
  await redis('SET', `room:${code}`, JSON.stringify(room), 'EX', '10800');
}
